...
| **Lake Day**<br>`iOS · SwiftUI` | Which lake, which day. Scores Washington lake days from live feeds — county bacteria and toxic-algae closures, water temp, weather, smoke AQI, drive-time crowds — with no backend and no API keys. Safety is never shaded: an active closure zeroes the score outright, every reading shows its age instead of implying freshness, and a broken feed degrades to "unknown" rather than a fake green. The state algae registry sits behind bot defense, so a daily Action drives real Chromium and commits the snapshot the app reads. | [Site](https://nano-ai.github.io/lake-day/) · [Code](https://github.com/Nano-AI/lake-day) |
